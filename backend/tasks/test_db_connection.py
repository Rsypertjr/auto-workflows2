...
try:
    ...
    print(
        "❌ Critical: Could not find 'app.main'. Ensure this script is placed in 'backend/tasks/'"
    )
    sys.exit(1)
...
async def verify_cluster_connection():
    ...
    try:
        ...
        if "asyncpg" not in DATABASE_URL:
            print(
                "💡 Tip: Missing '+asyncpg' driver modifier in your connection string."
            )
        elif "postgres-db" in DATABASE_URL:
            print(
                "💡 Tip: Ensure this script runs INSIDE docker. Host machines cannot resolve 'postgres-db'."
            )
        elif "localhost" in DATABASE_URL:
            print(
                "💡 Tip: Inside a container, 'localhost' looks inside itself, NOT the database container."
            )
